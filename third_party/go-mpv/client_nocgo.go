...
func ensureLoaded() error {
	loadOnce.Do(func() {
		libmpv, loadErr = loadLibrary()
		if loadErr != nil {
			return
		}
		defer func() {
			if recovered := recover(); recovered != nil {
				loadErr = fmt.Errorf("register libmpv functions: %v", recovered)
				libmpv = 0
			}
		}()
		registerLibFuncs()
...
func New() *Mpv {
	...
	m.handle = create()
	if m.handle == 0 {
		m.err = errors.New("mpv: mpv_create returned nil")
	}
	return m
}

func (m *Mpv) ready() error {
	if m == nil {
		return errors.New("mpv: nil client")
	}
	if m.err != nil {
		return m.err
	}
	...
	if m.handle == 0 {
		return errors.New("mpv: client handle is not initialized")
	}
	return nil
}
